...
export function NavSettings() {
  ...
  const loadSavedColors = () => {
    ...
    if (savedColors) {
      ...
      document.documentElement.style.setProperty(
        "--color-fort",
        parsedColors.fortColor
      );
      document.documentElement.style.setProperty(
        "--color-moyen",
        parsedColors.moyenColor
      );
      document.documentElement.style.setProperty(
        "--color-faible",
        parsedColors.faibleColor
      );
    }
  };
  ...
  const handleSubmitColors = () => {
    try {
      ...
      Cookies.set(
        "colorSettings",
        JSON.stringify({ fortColor, moyenColor, faibleColor })
      );
      setOpen(false);
    } catch (error) {
      console.error(error);
            ...
            <Drawer onOpenChange={setOpen} open={open}>
              ...
              <DrawerContent>
                <DrawerHeader>
                  ...
                  <DialogDescription>
                    Ici vous pouvez modifier les paramètres de
                    l&apos;application
                  </DialogDescription>
                </DrawerHeader>

                <div className="grid grid-cols-2 items-center gap-2 p-2 text-right">
